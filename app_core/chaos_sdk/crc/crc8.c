...
#define CRC8_FAST_BYTE_PROCEED(crc, data)																\
		crc8_maxim_table[(crc) ^ (data)]

#define CRC8_FAST_ARRAY_PROCEED(crc, data, len)															\
		while ((len)--) {																				\
			(crc) = CRC8_FAST_BYTE_PROCEED(crc, *(data)++);												\
		}
...
#undef CRC8_FAST_ARRAY_PROCEED

#endif /* _MY_CRC8_TABLE_CALC_ENA */

#ifdef _MY_CRC8_GENERIC_CALC_ENA

#define CRC8_SLOW_BYTE_PROCEED(crc, data)																\
		(crc) ^= (data);																				\
		for (u8 bit = 0; bit < 8; ++bit) {																\
			(crc) = ((crc) & 0x80U) ? (((crc) << 1U) ^ CRC8POLY) : ((crc) << 1U);						\
		}

#define CRC8_SLOW_ARRAY_PROCEED(crc, data, len)															\
	while ((len)--) {																					\
		CRC8_SLOW_BYTE_PROCEED(crc, *(data)++)															\
	}
...
{
	u8 crc = CRC8INIT;
	CRC8_SLOW_ARRAY_PROCEED(crc, data, len);
	return crc;
}
